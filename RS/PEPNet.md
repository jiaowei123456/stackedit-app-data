...
本质是多个多个全连接层（hidden_units），每个全连接层（hidden_units[i]）的输出都受到GNU的加权，每一场景的输出都加入outputs
```Python
class PPNet:  
    def __init__(self,  
                 multiples,  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjQ5MTgwNjE5LDYyMDY0NDUzOSwtMjA2Mz
UzMzc1MSwtMTA4ODM0MzkzNCwtMTIwMzUzMjY0NF19
-->