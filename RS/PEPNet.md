...
本质是多个全连接层（hidden_units），每个全连接层（hidden_units[i]）的输出都受到GNU的加权，每一层的输出都加入outputs
```Python
class PPNet:  
    def __init__(self,  
                 multiples,  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwODYxNjM4OTAsLTIwNjM1MzM3NTEsLT
EwODgzNDM5MzQsLTEyMDM1MzI2NDRdfQ==
-->