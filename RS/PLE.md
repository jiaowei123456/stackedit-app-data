...
#### 代码实现
```Python
share_output=[expert(task_fea[-1]).unsqueeze(1) for expert in self.share_experts[i]] # share_experts为layers_num层，每一层有shared_expert_num个全连接层，
```
### 4.3 Gate加权输出
#### 代码实现
```Python
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI0NjgyMTc0MywtMTU4Njc3NzUxMSwxOT
