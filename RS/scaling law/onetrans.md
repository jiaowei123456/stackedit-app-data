...
## 背景
1. 主流精排模型包含两个重要模块：（1）特征交叉：通过FM（DeepFM）或显式交叉网络（DCN，DCNv2，SENet，PEPNet）或特征组上的注意力机制（RankMixer）来学习非序列特征之间的高阶交叉。（2）序列建模：利用局部注意力或 Transformer 编码器将用户的多行为序列编码为具有候选意识的表示形式。（本质就是特征筛选压缩与有效表zheng）
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxOTUyMzU4NV19
-->