...
2. ONETRANS的基础架构实现了双向信息流动。它采用了一个统一的tokenizer，将序列特征和非序列特征都转换为一个Token序列或单一的Token，然后由一系列堆叠的 ONETRANS Block进行处理。
3. 为了适应推荐系统中各种不同的Token来源，与LLM中仅包含文本Token的情况不同，每个 ONETRAN 块采用了类似于 HiFormer的混合参数化方式。具体来说，所有序列Token共享一组 Q/K/V 和 FFN 权重，而每个非序列Token则接收标记特定的参数以保留其独特的语义。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAwNzI4ODEzOV19
-->