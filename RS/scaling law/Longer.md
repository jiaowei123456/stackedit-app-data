...
为了在对多个物料进行评分时提高推理效率，引入了一种 KV 缓存机制，该机制将用户行为标与针对候选者的全局标记之间的注意力计算分离开来。由于用户序列在不同候选人之间保持不变，其内部表示可以一次性计算并重复使用。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU1MTYxODkyMCwxMDczOTY0MDU5LDgwMz
